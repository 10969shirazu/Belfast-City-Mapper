    ...
    def button_Go(self):
        ...
        if _hops == 1:
            self.cursor.execute(f"""
                select distinct A.name, C.route_name, B.name 
                from network_nodes_belfast as A, network_nodes_belfast as B, 
                routes_belfast as C, routes_bus_belfast as D, routes_bus_belfast as E 
                WHERE A.stop_i=D.stop_i AND B.stop_i=E.stop_i and D.route_i=E.route_i 
                AND C.route_i=D.route_i AND A.name='{_fromstation}' AND B.name='{_tostation}';
            """)
            self.conn.commit()
            rows += self.cursor.fetchall()
